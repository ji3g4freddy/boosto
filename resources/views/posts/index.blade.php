    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                <!-- delete the post (uses the destroy method DESTROY /posts/{id} -->
                <!-- we will add this later since its a little more complicated than the other two buttons -->
                {{ Form::open(array('url' => 'verify_no/' . $value->id, 'class' => 'pull-right')) }}
                {{ Form::hidden('_method', 'PATCH') }}
                {{ Form::submit('審核不通過', array('class' => 'btn btn-warning')) }}
                {{ Form::close() }}

                {{ Form::open(array('url' => 'verify/' . $value->id, 'class' => 'pull-right')) }}
                {{ Form::hidden('_method', 'PATCH') }}
